````markdown
# S04 – eda_cli: HTTP-сервис качества датасетов (FastAPI)

Расширенная версия проекта `eda_cli` из Семинара 03.  
К существующему CLI-приложению для EDA добавлен HTTP-сервис на **FastAPI** с эндпоинтами `/health`, `/quality`, `/quality-from-csv` и новым эндпоинтом `/quality-flags-from-csv` (HW03/HW04).

---

## 🔗 Связь с S03

Проект в S04 основан на том же пакете `eda_cli`:

- сохраняется структура `src/eda_cli/` и CLI-команда `eda-cli`;  
- добавлен модуль `api.py` с FastAPI-приложением;  
- в зависимости добавлены `fastapi` и `uvicorn[standard]`.  

---

## 📦 Требования

- **Python 3.11+**  
- Uvicorn установлен в систему  
- Браузер (для Swagger UI `/docs`) или любой HTTP-клиент (curl, Postman, Hoppscotch)

---

## 🚀 Инициализация проекта

В корне проекта:

```bash
uv sync
````

Команда:

* создаст виртуальное окружение `.venv`;
* установит зависимости из `pyproject.toml` (включая FastAPI и Uvicorn);
* установит сам проект `eda-cli` в окружение.

---

## ▶️ Запуск CLI

CLI остаётся доступным:

```bash
uv run eda-cli overview data/example.csv
```

Параметры:

* `--sep` — разделитель (по умолчанию `,`)
* `--encoding` — кодировка файла (по умолчанию `utf-8`)

Полный EDA-отчёт:

```bash
uv run eda-cli report data/example.csv --out-dir reports
```

В каталоге `reports/` появятся:

* `report.md` — основной отчёт
* `summary.csv` — таблица по колонкам
* `missing.csv` — пропуски по колонкам
* `correlation.csv` — корреляционная матрица
* `top_categories/*.csv` — top-k категорий
* `hist_*.png` — гистограммы числовых колонок
* `missing_matrix.png` — визуализация пропусков
* `correlation_heatmap.png` — тепловая карта корреляций

---

## 🔧 Новые параметры команды `report` (HW03)

| Параметр              | Описание                                                               |
| --------------------- | ---------------------------------------------------------------------- |
| `--title`             | Заголовок отчёта в начале `report.md`                                  |
| `--max-hist-columns`  | Сколько числовых колонок включать в гистограммы                        |
| `--top-k-categories`  | Сколько top-значений отображать для категориальных признаков           |
| `--min-missing-share` | Порог доли пропусков: колонки выше порога выводятся в отдельный список |

---

## 🧪 Тестирование

Запуск тестов:

```bash
uv run pytest -q
```

---

# 🌐 HTTP-сервис (FastAPI)

HTTP-сервис реализован в модуле `eda_cli.api`.

### 🔹 Запуск Uvicorn

```bash
uv run uvicorn eda_cli.api:app --reload --port 8000
```

* `eda_cli.api:app` — путь до объекта FastAPI app
* `--reload` — автоматический перезапуск сервера при изменении кода
* `--port 8000` — порт сервиса (можно менять)

После запуска сервис доступен по адресу: **[http://127.0.0.1:8000](http://127.0.0.1:8000)**

---

## 📄 Эндпоинты сервиса

### 1️⃣ GET `/health` — проверка работоспособности

**Простейший health-check**

**Пример запроса:**

```bash
curl http://127.0.0.1:8000/health
```

**Пример ответа 200 OK:**

```json
{
  "status": "ok",
  "service": "dataset-quality",
  "version": "0.3.1"
}
```

---

### 2️⃣ GET `/docs` — Swagger UI

Документация и тестирование API: **[http://127.0.0.1:8000/docs](http://127.0.0.1:8000/docs)**

Через `/docs` можно:

* вызывать `GET /health`
* вызывать `POST /quality`
* вызывать `POST /quality-from-csv`
* вызывать `POST /quality-flags-from-csv`

---

### 3️⃣ POST `/quality` — анализ агрегированных признаков (старые флаги)

Эндпоинт принимает JSON с агрегированными характеристиками датасета:

**Пример запроса:**

```bash
curl -X POST "http://127.0.0.1:8000/quality" \
  -H "Content-Type: application/json" \
  -d '{"n_rows": 10000, "n_cols": 12, "max_missing_share": 0.15, "numeric_cols": 8, "categorical_cols": 4}'
```

**Пример ответа 200 OK:**

```json
{
  "ok_for_model": true,
  "quality_score": 0.85,
  "message": "Данных достаточно, модель можно обучать (по текущим эвристикам).",
  "latency_ms": 3.2,
  "flags": {
    "too_few_rows": false,
    "too_many_columns": false,
    "max_missing_share": false,
    "too_many_missing": false
  },
  "dataset_shape": {
    "n_rows": 10000,
    "n_cols": 12
  }
}
```

---

### 4️⃣ POST `/quality-from-csv` — оценка качества по CSV (старые флаги)

Эндпоинт принимает CSV-файл и возвращает **старые флаги качества**, вычисленные без новых эвристик:

**Пример запроса:**

```bash
curl -X POST "http://127.0.0.1:8000/quality-from-csv" \
  -F "file=@data/example.csv"
```

**Пример ответа:**

```json
{
  "ok_for_model": true,
  "quality_score": 0.72,
  "message": "CSV достаточно качественный для модели.",
  "latency_ms": 5.8,
  "flags": {
    "too_few_rows": false,
    "too_many_columns": false,
    "max_missing_share": false,
    "too_many_missing": true
  },
  "dataset_shape": {
    "n_rows": 5234,
    "n_cols": 18
  }
}
```

---

### 5️⃣ 🆕 POST `/quality-flags-from-csv` — расширенные флаги качества (HW03/HW04)

Эндпоинт принимает CSV-файл и возвращает **все флаги**, включая новые HW03-флаги, и рассчитывает **новое качество**:

**Пример запроса:**

```bash
curl -X POST "http://127.0.0.1:8000/quality-flags-from-csv" \
  -F "file=@data/example.csv"
```

**Пример ответа:**

```json
{
  "ok_for_model": true,
  "quality_score": 0.65,
  "message": "CSV достаточно качественный для модели.",
  "latency_ms": 6.91,
  "flags": {
    "too_few_rows": false,
    "too_many_columns": false,
    "max_missing_share": false,
    "too_many_missing": false,
    "has_high_ratio_of_unique_values": true,
    "has_outliers": true,
    "has_mixed_types": false,
    "has_imbalanced_categoricals": false,
    "has_many_repeated_rows": false
  },
  "dataset_shape": {
    "n_rows": 5234,
    "n_cols": 18
  }
}
```

**Описание новых флагов HW03:**

| Флаг                              | Описание                               |
| --------------------------------- | -------------------------------------- |
| `has_high_ratio_of_unique_values` | В колонке >80% уникальных значений     |
| `has_outliers`                    | >5% выбросов по правилу 1.5×IQR        |
| `has_mixed_types`                 | Колонка содержит значения разных типов |
| `has_imbalanced_categoricals`     | Категория >90% всех строк              |
| `has_many_repeated_rows`          | >10% повторяющихся строк               |

---

## 🗂 Структура проекта (упрощённо)

```
S04/
  eda-cli/
    pyproject.toml
    README.md
    src/
      eda_cli/
        __init__.py
        core.py       # EDA-логика, эвристики качества
        viz.py        # визуализации
        cli.py        # CLI (overview/report)
        api.py        # HTTP-сервис (FastAPI)
    tests/
      test_core.py   # тесты ядра
    data/
      example.csv     # учебный CSV для экспериментов
```